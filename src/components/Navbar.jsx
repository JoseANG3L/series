...
import { Search, Star, Menu, X, ChevronDown, LogOut, Settings, User as UserIcon, PlusCircle } from 'lucide-react';
              ...
              <div className="relative" ref={userMenuRef}>
                ...
                {isUserMenuOpen && (
                  <div className="absolute top-full right-0 mt-3 w-64 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden animate-fadeIn flex flex-col">
                    ...
                    {role === 'admin' && (
                      <button onClick={handleContenidoClick} className="text-left px-4 py-3 text-sm text-slate-300 hover:bg-slate-800 hover:text-white flex items-center gap-2 transition w-full">
                        <PlusCircle className="w-4 h-4" /> Crear Contenido
                      </button>
                    )}
      ...
      >
        <div className="min-h-full flex flex-col items-center justify-center py-20">
          <ul className="flex flex-col items-center gap-6 text-xl font-bold">
            ...
            {user && role === 'admin' && (
              <button onClick={handleContenidoClick} className="text-yellow-500 hover:text-yellow-400 font-bold transition">
                Crear Contenido
              </button>
            )}
          </ul>
