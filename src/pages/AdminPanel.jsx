...
import { Trash2, Edit, LayoutGrid, Film, Tv, Search, Filter, Calendar } from "lucide-react";
...
const AdminPanel = () => {
    ...
    const [editingItem, setEditingItem] = useState(null); // <--- ESTADO NUEVO: Qué estoy editando
    ...
    const handleEditClick = (item) => {
        setEditingItem(item); // 1. Ponemos el item en el 'hueco' del editor
        window.scrollTo({ top: 0, behavior: 'smooth' }); // 2. Subimos al formulario
    };

    const handleCancelEdit = () => {
        setEditingItem(null); // Limpiamos para volver a modo "Crear"
    };

    const handleSuccess = () => {
        fetchContent();       // Recargamos datos
        ...
        <div className="min-h-screen bg-[#0f172a] pt-24 pb-12 px-4 md:px-8 lg:px-16 text-white">
            <div className="max-w-7xl mx-auto space-y-12">
                ...
                <div className="space-y-4">
                    ...
                    {/* Pasamos los props nuevos al formulario */}
                    <ContentForm
                ...
                <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden shadow-2xl">
                    ...
                    <div className="p-6 border-b border-slate-800 flex flex-col lg:flex-row justify-between items-center gap-4 bg-slate-900/50">
                        <h2 className="text-xl font-bold flex items-center gap-2 whitespace-nowrap">
                            Inventario <span className="text-xs bg-slate-700 px-2 py-0.5 rounded-full text-slate-300">{filteredContent.length}</span>
                        </h2>
